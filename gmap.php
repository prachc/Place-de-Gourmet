...
if (strpos($uagent, 'Android') !== false) {
	...
	$upper = 83.5;
	$lower = 15;
}else if (strpos($uagent, 'iPhone') !== false){
	$uagent='iPhone';
	$upper = 88;
	$lower = 11;
}else {
...
<script type="text/javascript">
	...
	function initialize(ilat,ilng,imode,ihotelno) {
		if(device=="Android"||device=="iPhone") //using navigator.geolocation
		...
		//navigator.geolocation.getCurrentPosition(function (position){lat=position.coords.latitude;lng=position.coords.longitude});
		//var gps = google.gears.factory.create('beta.geolocation');
		//gps.getCurrentPosition(function (position){lat=position.coords.latitude;lng=position.coords.longitude});
		/*while(true){
...
<div id="map_canvas" style="width: 99.5%; height: <?php echo $upper?>%"></div>
<div id="lower" style="width: 99%; height: <?php echo $lower?>%; border:4px solid red;">
